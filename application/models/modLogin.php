...
class modLogin extends CI_Model {
	...
	public function selectKodeCab($txtusername)
	{
		$query = $this->db->where('username', $txtusername)
							->get('karyawan'); 
		return $query; 
	}

	public function selectnamacabang($kodecabb)
	{
		$query = $this->db->where('kodecabang', $kodecabb)
		->get('cabang'); 
		return $query; 
	}
