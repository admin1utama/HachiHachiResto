...
class PenerimaanBahan extends CI_Controller {
	...
	public function penerimaan()
	{
		if($this->input->post('btnSimpan')){
			...
			// looping dari cart
			$kdcabang 	= $this->session->userdata('codecabang'); 
			...
			for($i = 0; $i < $jumcart; $i+=1){
				...
				//echo $kodebahan;
				$this->modPemesananBahan->insert_dpenerimaan($nomernota,$kodebahan,$kodesatuan,$qtypesan,$qty,$hargabeli,$kdcabang, $nilaikonversi);
			}
			$data['datapemesananbahan'] = $this->modPemesananBahan->select_pemesanan();
			$this->load->view('showPenerimaan',$data);
