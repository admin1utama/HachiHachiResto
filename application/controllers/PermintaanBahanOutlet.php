...
class PermintaanBahanOutlet extends CI_Controller {
	...
	public function tambahPermintaan()
    {
		...
		if($this->input->post('btnSimpan')){
            ...
            $this->session->unset_userdata('cart');
            $params['datapemesananbahan'] = $this->modPermintaanBahanOutlet->select_permintaan();
            
			$this->load->view('showPermintaanBahanOutlet',$params);
		}
    ...
    public function getDetailCabang()
    {
		...
		foreach($dtcabang->result() as $row)
		{
			echo "<tr>";
				//echo "<td>Alamat </td>";	
				echo "<td class='judullabel' style='font-size: 20px;'>".$row->alamat."</td>";	
			echo "</tr>";
			echo "<tr>";
				//echo "<td>Kota </td>";	
	...
	public function responsePermintaan() {
		$nomernota 	= $this->input->post('kode'); 
		$cabangpenerima = $this->input->post("cabangpenerima"); 
		$cabangpengirim = $this->input->post("txtcabang"); 
		
		if($cabangpenerima == $cabangpengirim) {
			$data['message'] 			= "Cabang Pengirim Tidak Boleh sama dengan Cabang Penerima"; 
			$data['datacabang']  	    = $this->modCabang->select_cabang();
			$data['datapemesananbahan'] = $this->modPermintaanBahanOutlet->select_permintaan();
			$this->load->view('showPermintaanBahanOutlet',$data);	
		}
		else {
			$this->modPermintaanBahanOutlet->sukses($nomernota, $cabangpenerima, $cabangpengirim);
			$data['datacabang']  	    = $this->modCabang->select_cabang();
			$data['datapemesananbahan'] = $this->modPermintaanBahanOutlet->select_permintaan();
			$this->load->view('showPermintaanBahanOutlet',$data);	
		}
	...
	public function getStok()
	{
		...
		$idbahan 	= $this->input->post("idbahan");
		$bhnbaku 	= $this->modPermintaanBahanOutlet->selectstok_bycabang($kodecabang, $idbahan);
	...
	public function batal_permintaan($nomernota)
	{
		$this->modPermintaanBahanOutlet->batal($nomernota);
		$this->index();
	}

	public function sukses_permintaan($nomernota)
	{
		$this->modPermintaanBahanOutlet->sukses($nomernota);
		$this->index();
	}

	public function detail($nomernota)
	{
		$data['header']	= $this->modPermintaanBahanOutlet->selectjudulheader($nomernota);
		$data['detail']	= $this->modPermintaanBahanOutlet->selectdataforcart($nomernota);
		$data['datapemesananbahan'] = $this->modPermintaanBahanOutlet->select_permintaan_header();
